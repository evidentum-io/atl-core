...
fn test_verify_rfc3161_anchor_integration() {
    ...
    let result = verify_rfc3161_anchor_impl(timestamp, &token, &FREETSA_HASH, None);
...
pub(crate) fn decode_freetsa_root_cert() -> x509_cert::Certificate {
    use base64::Engine;
    use der::Decode;
