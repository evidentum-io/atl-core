...
impl AtlError {
    ...
    pub fn is_format_error(&self) -> bool {
        matches!(
            self,
            AtlError::InvalidHash(_)
                | AtlError::InvalidSignature(_)
                | AtlError::InvalidPublicKey(_)
                | AtlError::InvalidCheckpointFormat(_)
                | AtlError::InvalidCheckpointMagic
                | AtlError::InvalidReceipt(_)
                | AtlError::Json(_)
                | AtlError::Base64Decode(_)
                | AtlError::HexDecode(_)
                | AtlError::InvalidUuid(_)
        )
    }
    ...
    pub fn is_proof_error(&self) -> bool {
        matches!(
            self,
            AtlError::LeafIndexOutOfBounds { .. }
                | AtlError::TreeSizeMismatch { .. }
                | AtlError::InvalidProofPath { .. }
                | AtlError::InclusionProofInvalid
                | AtlError::ConsistencyProofInvalid
                | AtlError::MissingNode { .. }
        )
    }
}
...
impl From<hex::FromHexError> for AtlError {
    fn from(e: hex::FromHexError) -> Self {
        AtlError::HexDecode(e.to_string())
    }
}
...
impl From<base64::DecodeError> for AtlError {
    fn from(e: base64::DecodeError) -> Self {
        AtlError::Base64Decode(e.to_string())
    }
}
...
impl From<uuid::Error> for AtlError {
    fn from(e: uuid::Error) -> Self {
        AtlError::InvalidUuid(e.to_string())
    }
}
...
mod tests {
    ...
    fn test_error_display() {
        let err = AtlError::LeafIndexOutOfBounds {
            index: 100,
            tree_size: 50,
        };
        assert_eq!(
    ...
    fn test_tree_size_mismatch_display() {
        let err = AtlError::TreeSizeMismatch {
            expected: 1000,
            actual: 500,
        };
    ...
    fn test_origin_mismatch_display() {
        ...
        assert_eq!(
            err.to_string(),
            "origin mismatch: expected origin-a, got origin-b"
        );
    }
    ...
    fn test_is_verification_failure() {
        ...
        assert!(AtlError::OriginMismatch {
            expected: "a".into(),
            actual: "b".into()
        }
        ...
        assert!(!AtlError::Json(serde_json::from_str::<()>("x").unwrap_err())
            .is_verification_failure());
    }
    ...
    fn test_is_proof_error() {
        ...
        assert!(AtlError::LeafIndexOutOfBounds {
            index: 0,
            tree_size: 0
        }
        .is_proof_error());
        assert!(AtlError::TreeSizeMismatch {
            expected: 1,
            actual: 2
        }
        .is_proof_error());
        assert!(AtlError::InvalidProofPath {
            expected: 5,
            actual: 3
        }
        .is_proof_error());
